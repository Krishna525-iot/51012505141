...
//	lcd_puts(3, 1, (int8_t*) "SENSR");
	lcd_puts(3, 1, (int8_t*) "LAMP");
	lcd_puts(1, 11, (int8_t*) "ENDO");
	lcd_puts(2, 11, (int8_t*) "DEPTH");
	lcd_puts(3, 11, (int8_t*) "     .->");
}
...
//	lcd_puts(3, 7, sensor_state ? (int8_t*) "ON " : (int8_t*) "OFF");
	...
	// Update lamp state
	lcd_puts(3, 7, lamp_state ? (int8_t*) "ON " : (int8_t*) "OFF");

	// Update ENDO state
	lcd_puts(1, 17, endo_state ? (int8_t*) "ON " : (int8_t*) "OFF");

	// Update depth state
	lcd_puts(2, 17, depth_state ? (int8_t*) "ON " : (int8_t*) "OFF");
}
...
void handle_depth_action(void) {
	uint32_t press_duration = HAL_GetTick() - button_states[4].press_start_time;

	if (press_duration > LONG_PRESS_THRESHOLD) {
		uint32_t elapsed_time = HAL_GetTick() - system_start_time;
		display_uptime_and_return(elapsed_time);
	} else {
...
const CursorAction cursor_actions[] = { { 1, 7, increment_intensity,decrement_intensity },   // INTEN
		{ 2, 7, toggle_color_mode, decrement_color_mode },    // COLOR
		{ 3, 7, turn_on_lamp, turn_off_lamp }, 				  // LAMP
		{ 1, 17, turn_on_endo, turn_off_endo },              // ENDO
		{ 2, 17, turn_on_depth, turn_off_depth },             // DEPTH
		{ 3, 17, enable_sensor, disable_sensor }             // SENSR
};

const uint8_t num_positions = sizeof(cursor_actions)
		/ sizeof(cursor_actions[0]);

const uint8_t cursor_positions[][2] = { { 1, 0 }, { 2, 0 }, { 3, 0 }, { 1, 10 },
		{ 2, 10 }, { 3, 10 } };
